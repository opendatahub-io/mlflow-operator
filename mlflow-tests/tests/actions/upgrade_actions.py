...
def action_log_upgrade_text_artifact(test_context: TestContext) -> None:
    ...
    artifact_dir = tempfile.mkdtemp(prefix="mlflow-upgrade-artifact-")
    ...
    if os.path.commonpath([artifact_dir_realpath, target_path_realpath]) != artifact_dir_realpath:
        raise ValueError(f"artifact_file must stay within the temporary artifact directory: {target_name!r}")
...
def action_create_upgrade_trace(test_context: TestContext) -> None:
    """Create the current upgrade trace with optional session metadata and attachment."""
...
def action_collect_upgrade_trace_observations(test_context: TestContext) -> None:
    """Collect persisted trace data and attachment payloads for the active upgrade case."""
    ...
    for trace in traces:
        ...
        # MLflow exposes trace attachment bytes through the get-trace-artifact HTTP
        # endpoint. The Python client does not currently provide a dedicated helper
        # for downloading an individual trace attachment by request/attachment id.
