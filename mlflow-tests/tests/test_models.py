...
class TestRegisteredModels(TestBase):
    ...
    test_scenarios: ClassVar[list[TestData]] = [
        ...
        TestData(
            test_name="Validate that user with CREATE and DELETE permissions can delete registered model",
            user_info=UserInfo(workspace=Config.WORKSPACES[0], verbs=[KubeVerb.CREATE, KubeVerb.DELETE], resource_types=[ResourceType.REGISTERED_MODELS]),
            workspace_to_use=Config.WORKSPACES[0],
            test_steps = [
