...
class TestExperiments(TestBase):
    ...
    test_scenarios: ClassVar[list[TestData]] = [
        ...
        TestData(
            test_name="Validate that user with CREATE and DELETE permissions can delete experiment",
            user_info=UserInfo(workspace=Config.WORKSPACES[0], verbs=[KubeVerb.CREATE, KubeVerb.DELETE], resource_types=[ResourceType.EXPERIMENTS]),
            workspace_to_use=Config.WORKSPACES[0],
            test_steps = [
