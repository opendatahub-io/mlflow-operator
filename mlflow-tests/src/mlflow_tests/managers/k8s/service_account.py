...
class ServiceAccountManager:
    ...
    def _create_service_account(
        ...
        try:
            ...
            if e.status == 409:  # Already exists, return existing one
                ...
                raise Exception(f"Failed to create service account due to: {e}")
    ...
    def _get_token_via_token_request(
        ...
        for attempt in range(max_retries):
            try:
                ...
                token_request = client.V1TokenRequestSpec(
    ...
    # Maintain backwards compatibility by aliasing the old method name
    def create_sa_and_get_secret(self, sa_name: str, other_info: str) -> tuple[str, str]:
        """Legacy method name for backwards compatibility. Use create_sa_and_get_token instead."""
        logger.warning("create_sa_and_get_secret is deprecated, use create_sa_and_get_token instead")
        return self.create_sa_and_get_token(sa_name, other_info)
