...
- `operator-chaos.yml` - Offline operator-chaos upgrade-risk gate. It validates `chaos/knowledge/mlflow.yaml`, runs `operator-chaos preflight --local`, diffs the base and PR knowledge models with `operator-chaos diff --breaking`, compares the checked-in MLflow CRD schema with `operator-chaos diff-crds`, previews generated upgrade experiments with `operator-chaos simulate-upgrade --dry-run`, and writes a GitHub job summary. This workflow is intentionally asset-focused and does not create a cluster or execute live chaos experiments.
- `operator-chaos-pr-comment.yml` - Trusted follow-up workflow triggered by `workflow_run` after `operator-chaos.yml` completes. It re-runs the same offline operator-chaos checks against the PR head and base SHAs in trusted workflow code and then updates the sticky PR comment with base-repository permissions, which avoids the fork-token write restriction that applies to the pull-request workflow itself while keeping the published comment content under trusted control.
