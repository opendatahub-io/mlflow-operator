...
func TestRenderChart_ServiceMonitorWithTLSConfig(t *testing.T) {
	...
	deployment := findObject(objs, deploymentKind)
...
func TestRenderChart_NetworkPolicy(t *testing.T) {
	...
	np := findObject(objs, "NetworkPolicy")
...
func findObject(objs []*unstructured.Unstructured, kind string) *unstructured.Unstructured {
	for _, obj := range objs {
		if obj.GetKind() == kind {
			return obj
		}
